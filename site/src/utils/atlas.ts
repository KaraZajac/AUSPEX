 ...
 * builds a typed index, and exposes query helpers used by Astro pages.
 *
 * Types are intentionally loose — the corpus is hand-curated and the
 * occasional optional field will be missing. The loader is defensive
 * (never throws on a malformed entry, only warns to stderr).
 */
...
export class Atlas {
  ...
  constructor() {
    this.nationStates = loadDir<NationState>('nation-states');
    this.services = loadDir<Service>('services');
    this.doctrines = loadDir<Doctrine>('doctrines');
    this.actors = loadDir<Actor>('actors');
    this.events = loadDir<AuspexEvent>('events');
    this.sources = loadDir<Source>('sources');
    this.timelineMarkers = loadDir<TimelineMarker>('timeline-markers');
    this.sectors = loadSectors();
    this.policyActions = loadDir<PolicyAction>('policy-actions');
    this.targets = loadDir<Target>('targets');

    // Load malware-lineage table (single file, distinct from loadDir).
    this.malwareLineage = new Map();
    this.malwareLineageGroup = new Map();
    try {
      ...
      for (const fam of doc?.families ?? []) {
        ...
        for (const key of [fam.name, ...(fam.aliases ?? [])]) {
          ...
          this.malwareLineage.set(k, fam);
          if (lg) this.malwareLineageGroup.set(k, lg);
        }
      }
    } catch (err) {
    ...
    // Attach MITRE ATT&CK techniques and malware per actor from the
    // extracted caches. Built by tools/extract-mitre-ttps.ts; if
    // missing, actors silently get nothing and the engines fall back.
    const here = dirname(fileURLToPath(import.meta.url));
    ...
    try {
      ...
      if (malPath) {
        ...
        for (const actor of this.actors.values()) {
          const g = actor.external_refs?.mitre_attack;
          if (g && mal[g]) {
            actor.mitre_malware = mal[g];
            for (const name of mal[g]) {
...
function byStartDateDesc(a: AuspexEvent, b: AuspexEvent): number {
