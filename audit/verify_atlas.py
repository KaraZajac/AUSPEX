...
def check_backlog():
    ...
    qc_part = sum(1 for e in events.values() if (e.get("qc") or {}).get("level") == "partial")
    qc_src  = sum(1 for e in events.values() if (e.get("qc") or {}).get("level") == "sources-only")
    add("qc-coverage","INFO","events",
        f"verified: {qc_full} full + {qc_part} partial + {qc_src} sources-only of {len(events)} events "
        f"({100*(qc_full+qc_part+qc_src)//max(len(events),1)}%) — see docs/CORPUS-VERIFICATION-PLAN.md")
