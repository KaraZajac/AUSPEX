...
print(f"  cross-check: this actor-level mean DiD ({statistics.mean(r[5] for r in rows):+.2f}) does not cohere with the state-level")
print(f"  cut (per-action ≈ 0; clustered n=4 weakly negative — see deterrence.py). Disagreement at N={len(rows)}")
print(f"  is the signature of designs too weak for the question, not of a real effect.")
...
print("   this granularity. The control is also CONTAMINATED: the rest-of-corpus pool contains other NAMED")
print("   actors inside their own treatment windows (~27% of the control in some windows), so even the DiD")
print("   direction is unidentified. Do NOT claim actor-level deterrence from this corpus — the STATE-level")
print("   cut (clustered effective-N view), with more events per unit, is the defensible level of analysis.")
