...
time (median $+3$ years; 75\% of dated pairs postdate publication). Second, doctrine is
...
source; targets; and \code{doctrine\_links}. Every load-bearing claim links to a primary source
...
$+3$ years and 75\% of pairs \emph{postdate} the document's publication (Figure~\ref{fig:precedence});
...
75\% follow). The small right-tail overflow is foundational doctrines (e.g. Israel's Begin Doctrine,
...
is overwhelmingly data-theft (87\%); the profit-motivated buckets are unmistakable --- \code{criminal}
66\% money, DPRK 51\% money (the only \emph{state} above 25\% on money, a signature of the
...
sectors/countries, initial vectors, incident types, MITRE ATT\&CK techniques and co-occurrence pairs,
...
the split, the fair cold-start comparator --- attribution holds 56.2\% top-5 and doctrine 74.8\%;
